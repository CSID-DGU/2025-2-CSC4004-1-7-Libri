...
import { validateAPIResponse, getDefaultAIData, validateModelType, getErrorMessage, retryWithBackoff } from '../utils/dataValidation';
...
export default function StockDetail({ stockName, investmentStyle, onBack }: StockDetailProps) {
  ...
  const getMockPredictionResult = (modelType: string) => {
    const mockResults: Record<string, any> = {
      'model2': {
        ...
        gpt_explanation: 'A2C 모델 분석: 공격적인 투자 전략을 바탕으로 현재 매수 타이밍으로 판단됩니다. 단기 변동성이 있으나 상승 모멘텀이 강합니다.',
        technical_indicators: { EMA12: 61500, EMA26: 60800, MACD: 0.5, RSI: 65, Volume: 1500000 }
      },
      'marl': {
        signal: 'hold',
        confidence_score: 0.68,
        gpt_explanation: 'MARL 4-Agent 모델 분석: 단기/장기/위험/감성 에이전트가 종합 분석한 결과, 현재 보유 전략이 적절합니다. 시장 불확실성을 고려한 신중한 접근이 필요합니다.',
        technical_indicators: { SMA20: 62000, MACD: 0.3, RSI: 58, Stoch_K: 70, Volume: 1200000 }
      },
      'model3': {
        signal: 'hold',
        confidence_score: 0.82,
        gpt_explanation: 'MARL 3-Agent 모델 분석: 안정적인 수익을 목표로 하는 전략으로, 현재 보유가 최적입니다. 리스크를 최소화하며 장기적 관점에서 접근하세요.',
        technical_indicators: { DebtRatio: 45.3, ROE: 15, PER: 12, PBR: 1.2, DividendYield: 3.5 }
      }
    };
  ...
  const getFallbackRecommendation = (modelType: string) => {
    const fallbackData: Record<string, any> = {
      'model2': {
        recommendation: '매수',
        aiExplanation: '공격적 투자 성향에 맞는 분석을 진행 중입니다. 높은 수익을 추구하는 전략으로 접근하세요.',
        indicators: {}
      },
      'marl': {
        recommendation: '보유',
        aiExplanation: '균형잡힌 투자 전략을 바탕으로 분석 중입니다. 리스크와 수익의 균형을 고려한 접근이 필요합니다.',
        indicators: {}
      },
      'model3': {
        recommendation: '보유',
        aiExplanation: '안정적인 투자 전략에 맞는 분석을 진행 중입니다. 장기적 관점에서 안전한 투자를 추천합니다.',
        indicators: {}
      }
    };
    ...
    const loadAIAnalysis = async () => {
      try {
        ...
        setAiData({
          ...
          aiExplanation: result.gpt_explanation || '현재 시장 상황을 종합적으로 분석한 결과입니다.',
