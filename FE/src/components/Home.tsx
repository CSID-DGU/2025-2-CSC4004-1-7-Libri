    ...
    return (
        <div className="basis-0 content-stretch flex font-['Pretendard:Medium',sans-serif] gap-[4px] grow items-center leading-[0] min-h-px min-w-px not-italic relative shrink-0 text-nowrap tracking-[0.16px]">
            ...
            <div className="flex flex-col justify-center relative shrink-0 text-[#1fa9a4] text-[0px]">
                <p className="font-['Pretendard:Bold',sans-serif] leading-[1.5] not-italic text-[16px] text-nowrap tracking-[0.16px] whitespace-pre">
...
function Frame10({ profit, profitRate }: { profit: number; profitRate: number }) {
    ...
    return (
        <div className="basis-0 content-stretch flex font-['Pretendard:Medium',sans-serif] gap-[4px] grow items-center leading-[0] min-h-px min-w-px not-italic relative shrink-0 tracking-[0.16px]">
            ...
            <div
                className="flex flex-col justify-center relative shrink-0 text-[0px] text-nowrap"
...
function SKHynixLogo() {
    return (
        <div className="bg-white overflow-clip relative rounded-[100px] w-[36px] h-[36px] shrink-0">
            <div
                className="absolute h-[23px] left-[calc(50%+2px)] top-[calc(50%+0.5px)] translate-x-[-50%] translate-y-[-50%] w-[28px]"
                data-name="ci_logo_mark-02 1"
            >
                <div className="absolute inset-0 mix-blend-darken overflow-hidden pointer-events-none">
                    <img
                        alt=""
                        className="absolute h-[131.43%] left-0 max-w-none top-[-31.43%] w-[104.55%]"
                        src={imgCiLogoMark021}
                    />
                </div>
            </div>
        </div>
...
function Frame2({ profit, profitRate }: { profit: number; profitRate: number }) {
    ...
    return (
        <div className="content-stretch flex gap-[4px] items-start relative shrink-0">
            <div
                className="flex flex-col font-['Pretendard:Medium',sans-serif] justify-center leading-[0] not-italic relative shrink-0 text-[12px] text-center text-nowrap tracking-[0.24px]"
                style={{ color: profitColor }}
            >
...
function StockItem({
    ...
    return (
        <div
            className="bg-[#f2f4f8] relative rounded-[16px] shrink-0 w-full cursor-pointer"
            onClick={onClick}
        >
...
export default function Home({
    ...
    const totalProfitRate =
        stocks.length > 0
            ? (totalProfit /
                  stocks.reduce((sum, stock) => sum + (stock.totalValue - stock.profit), 0)) *
              100
            : 0;
