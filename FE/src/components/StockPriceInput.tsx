...
import svgPaths from "../imports/svg-c5sb4l7ys3";
...
function Frame9({ price, setPrice }: { price: string; setPrice: (value: string) => void }) {
    return (
...
function Frame10({ price, setPrice }: { price: string; setPrice: (value: string) => void }) {
    return (
        <div className="content-stretch flex gap-[6px] items-center relative shrink-0">
            <Frame9 price={price} setPrice={setPrice} />
        </div>
    );
}

function Component5({ price, setPrice }: { price: string; setPrice: (value: string) => void }) {
    return (
        <div
        ...
        >
            <div className="flex flex-row items-center size-full">
                <div className="box-border content-stretch flex items-center p-[12px] relative w-full">
                    <Frame10 price={price} setPrice={setPrice} />
                </div>
            </div>
        </div>
    );
...
function Frame8({ price, setPrice }: { price: string; setPrice: (value: string) => void }) {
    return (
        <div className="content-stretch flex flex-col gap-[20px] items-start relative shrink-0 w-full">
            <Component4 />
            <Frame6 price={price} setPrice={setPrice} />
        </div>
    );
}

function Frame5({ price, setPrice }: { price: string; setPrice: (value: string) => void }) {
    return (
        <div className="box-border content-stretch flex flex-col gap-[24px] items-center justify-center px-0 py-[16px] relative shrink-0 w-full">
            <Frame8 price={price} setPrice={setPrice} />
        </div>
    );
}
    ...
    return (
        <div className="bg-white relative size-full min-h-screen" data-name="온보딩">
            <div className="absolute content-stretch flex flex-col items-start left-1/2 top-[52px] translate-x-[-50%] w-full max-w-[375px]">
                ...
                <Frame5 price={price} setPrice={setPrice} />
            </div>
            ...
            <div className="absolute bottom-[50px] box-border content-stretch flex flex-col gap-[10px] items-start left-1/2 translate-x-[-50%] px-[20px] py-0 w-full max-w-[375px]">
                <button
                    ...
                    className={`${
                        price.trim() ? "bg-[#1FA9A4]" : "bg-[#d0d1d4]"
