...
from ai_wrapper import ai_service  # ✅ A2C / MARL AI 서비스 래퍼
...
# --------------------------------------------------------
# ✅ 브릿지 엔드포인트: /predict/{mode}
#   - FE: /predict/marl, /predict/a2c 로 요청
#   - BE: ai_service.predict_today() 로 연동 (A2C / MARL)
# --------------------------------------------------------
@app.post(
    "/predict/{mode}",
    dependencies=[Depends(verify_api_key)],
...
async def legacy_predict(
    ...
    """
    프론트엔드 호환용 브릿지:
      - /predict/marl → 내부적으로 A2C/MARL AI 서비스 호출
      - /predict/a2c  → 내부적으로 A2C/MARL AI 서비스 호출

    예상 요청 예시 (FE/src/api/client.ts - predictByInvestmentStyle):
      POST /predict/marl
      {
    ...
    # 기본 투자 성향 (FE에서 안 보내줄 수 있으니 기본값 aggressive)
    ...
    signal = action_en.lower()                # buy / sell / hold  → FE translateSignal 과 매칭
